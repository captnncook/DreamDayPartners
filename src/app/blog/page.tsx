        ...
        {BLOG_POSTS.map((post) => (
          <Link key={post.slug} href={`/blog/${post.slug}`} className="dash-row" style={{ padding: "1.375rem 0.25rem", alignItems: "flex-start" }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div className="ddp-section-label" style={{ marginBottom: "var(--space-2)" }}>{post.category}</div>
              <h2 className="font-serif" style={{ fontSize: "var(--text-3xl)", fontWeight: 700, letterSpacing: "-0.01em", lineHeight: 1.3, margin: 0 }}>
                {post.title}
              </h2>
