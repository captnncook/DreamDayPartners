...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const vendor = await prisma.vendor.findUnique({ where: { id }, select: { photos: true, coverPhoto: true } });
  if (!vendor) return NextResponse.json({ urls: [], coverUrl: null });
  ...
  return NextResponse.json({ urls, coverUrl });
}
