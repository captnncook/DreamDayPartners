...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (vendor.coverPhoto) {
    try { await deleteFile(vendor.coverPhoto); } catch { /* ignore */ }
  }

  const ext = file.name.split(".").pop() ?? "jpg";
  const fileKey = `vendors/${id}/cover-${uuidv4()}.${ext}`;
  ...
  await prisma.vendor.update({ where: { id }, data: { coverPhoto: fileKey } });
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (vendor.coverPhoto) {
    try { await deleteFile(vendor.coverPhoto); } catch { /* ignore */ }
    await prisma.vendor.update({ where: { id }, data: { coverPhoto: null } });
  }
