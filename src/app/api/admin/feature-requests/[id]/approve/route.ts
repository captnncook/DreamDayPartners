import { NextResponse } from "next/server";
import { prisma } from "@/lib/prisma";
import { getSession } from "@/lib/session";

import { withErrorLogging } from "@/lib/apiErrorLogging";
async function POSTImpl(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  const user = await getSession();
  if (!user || user.role !== "admin") return NextResponse.json({ error: "Geen toegang" }, { status: 403 });

  const { id } = await params;
  const request = await prisma.vendorFeatureRequest.findUnique({ where: { id } });
  if (!request) return NextResponse.json({ error: "Verzoek niet gevonden" }, { status: 404 });
  if (request.status !== "pending") return NextResponse.json({ error: "Verzoek is al afgehandeld" }, { status: 409 });

  const vendor = await prisma.vendor.findUnique({ where: { id: request.vendorId }, select: { extraModules: true } });
  if (!vendor) return NextResponse.json({ error: "Leverancier niet gevonden" }, { status: 404 });

  const nextExtraModules = Array.from(new Set([...vendor.extraModules, request.moduleKey]));

  const [updatedRequest] = await prisma.$transaction([
    prisma.vendorFeatureRequest.update({
      where: { id },
      data: { status: "approved", decidedAt: new Date(), decidedBy: user.email ?? user.id },
    }),
    prisma.vendor.update({
      where: { id: request.vendorId },
      data: { extraModules: nextExtraModules },
    }),
  ]);

  return NextResponse.json({ request: updatedRequest });
}

export const POST = withErrorLogging(POSTImpl);
