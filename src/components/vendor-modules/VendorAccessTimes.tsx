        ...
        {slots.map(slot => (
          <div key={slot.id}>
            {editingId === slot.id ? (
              ...
              <div style={{ background: "var(--accent)", border: "1px solid var(--border)", borderRadius: "10px", padding: "0.625rem 1rem", display: "flex", alignItems: "center", gap: "var(--space-5)" }}>
                <div style={{ minWidth: "3rem", textAlign: "center", background: "var(--primary)", color: "white", borderRadius: "6px", padding: "0.2rem 0.375rem", fontSize: "var(--text-base)", fontWeight: 700, fontVariantNumeric: "tabular-nums" }}>
                  {slot.accessTime}
                </div>
                <div className="flex-1 min-w-0">
                  <div style={{ fontWeight: 600, fontSize: "var(--text-md)" }}>{slot.vendorType}</div>
  ...
  return (
    <div style={{ background: "var(--accent)", border: "1px solid var(--border)", borderRadius: "10px", padding: "0.875rem", marginBottom: "var(--space-5)" }} className="space-y-2">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label style={{ fontSize: "var(--text-sm)", color: "var(--muted)", marginBottom: "var(--space-1)", display: "block" }}>Toegang vanaf *</label>
          <input type="time" value={form.accessTime} onChange={e => setForm(f => ({ ...f, accessTime: e.target.value }))} style={inputStyle} />
        </div>
        <div>
          <label style={{ fontSize: "var(--text-sm)", color: "var(--muted)", marginBottom: "var(--space-1)", display: "block" }}>Weg vóór</label>
