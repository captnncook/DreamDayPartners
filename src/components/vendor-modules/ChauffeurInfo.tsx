    ...
    <div className="ddp-card">
      ...
      {editing ? (
        <div className="space-y-2">
          <p style={{ fontSize: "var(--text-sm)", fontWeight: 600, color: "var(--muted)", textTransform: "uppercase", letterSpacing: "0.04em" }}>Chauffeur</p>
          ...
          <p style={{ fontSize: "var(--text-sm)", fontWeight: 600, color: "var(--muted)", textTransform: "uppercase", letterSpacing: "0.04em", marginTop: "var(--space-3)" }}>Vervanger bij ziekte</p>
          <div className="grid grid-cols-2 gap-2">
          ...
          <div className="flex gap-2">
            <button onClick={saveData} style={{ background: "var(--primary)", color: "white", border: "none", borderRadius: "8px", padding: "0.3rem 0.75rem", cursor: "pointer", display: "flex", alignItems: "center", gap: "var(--space-1)", fontSize: "var(--text-base)" }}>
        ...
        <div style={{ display: "grid", gap: "var(--space-4)" }}>
          <div style={{ background: "var(--accent)", border: "1px solid var(--border)", borderRadius: "10px", padding: "0.75rem 1rem" }}>
            <div style={{ fontSize: "var(--text-xs)", fontWeight: 600, color: "var(--muted)", textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: "var(--space-2)" }}>Chauffeur</div>
            <div style={{ fontWeight: 600, fontSize: "var(--text-lg)" }}>{data.name}</div>
            {data.phone && (
              <a href={`tel:${data.phone}`} style={{ display: "inline-flex", alignItems: "center", gap: "var(--space-2)", marginTop: "var(--space-2)", color: "var(--primary)", fontSize: "var(--text-md)", fontWeight: 600, textDecoration: "none" }}>
          ...
          {data.backupName && (
            <div style={{ background: "var(--accent)", border: "1px solid var(--border)", borderRadius: "10px", padding: "0.75rem 1rem" }}>
              <div style={{ fontSize: "var(--text-xs)", fontWeight: 600, color: "var(--muted)", textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: "var(--space-2)" }}>Vervanger bij ziekte</div>
              <div style={{ fontWeight: 600, fontSize: "var(--text-lg)" }}>{data.backupName}</div>
              {data.backupPhone && (
                <a href={`tel:${data.backupPhone}`} style={{ display: "inline-flex", alignItems: "center", gap: "var(--space-2)", marginTop: "var(--space-2)", color: "var(--primary)", fontSize: "var(--text-md)", fontWeight: 600, textDecoration: "none" }}>
                  <Phone className="w-3.5 h-3.5" /> {data.backupPhone}
                </a>
              )}
            </div>
          )}
          {data.notes && <p style={{ fontSize: "var(--text-base)", color: "var(--muted)", margin: 0 }}>{data.notes}</p>}
        </div>
      ) : (
