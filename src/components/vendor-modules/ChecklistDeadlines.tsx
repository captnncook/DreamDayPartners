  ...
  return (
    <div className="card" style={{ padding: "1.5rem" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "var(--space-6)" }}>
        ...
        <Link href={`/weddings/${weddingId}/tasks`} style={{ fontSize: "var(--text-base)", color: "var(--primary)", textDecoration: "none" }}>Alle taken →</Link>
      </div>

      {tasks.length === 0 ? (
        <p style={{ fontSize: "var(--text-md)", color: "var(--muted)", fontStyle: "italic" }}>Geen taken gekoppeld.</p>
      ) : (
          ...
          {open.map(t => (
            <div key={t.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0.625rem 0.75rem", background: "var(--blush-soft)", borderRadius: "0.5rem" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)" }}>
                ...
                <span style={{ fontSize: "var(--text-md)", color: "var(--charcoal)" }}>{t.title}</span>
              </div>
              {t.dueDate && <span style={{ fontSize: "var(--text-sm)", color: "var(--muted)" }}>{fmt(t.dueDate)}</span>}
            </div>
          ))}
          {done.length > 0 && (
            <div style={{ fontSize: "var(--text-sm)", color: "var(--muted)", paddingTop: "0.25rem" }}>
